...
const searchTokens = (value) => {
  ...
  return [...new Set(tokens.flatMap((token) => [token, ...(SEARCH_ALIASES[token] || [])]))];
};

const scoreOccupationMatch = (occupation, query) => {
  const queryTokens = searchTokens(query);
  if (!queryTokens.length) return 1;

  const title = occupation.name.toLowerCase();
  const titleTokens = searchTokens(occupation.name);
  if (title.includes(query.toLowerCase().trim())) return 100;

  const matches = queryTokens.filter((token) => titleTokens.includes(token));
  return matches.length;
};

const createTypedOccupation = (value) => {
  const name = value.trim();
  if (!name) return null;

  return {
    id: `typed-${name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`,
    name,
    category: "Closest model match",
  };
};
...
const Onboarding = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return occupations
      .map((o) => ({ occupation: o, score: scoreOccupationMatch(o, q) }))
  ...
  useEffect(() => {
    ...
    if (occupation.id?.startsWith("typed-")) return;
    if (search.trim() && scoreOccupationMatch(occupation, search) === 0) {
      setOccupation(null);
    }
  }, [occupation, search]);

  const typedOccupation = createTypedOccupation(search);
  const selectedOccupation = occupation || (step === 2 && search.trim() ? filtered[0] || typedOccupation : null);

  const handleCareerSearch = (value) => {
    setSearch(value);
    setOccupation(createTypedOccupation(value));
  };

  const goNext = () => {
    if (step === 2 && !occupation && selectedOccupation) {
          ...
          {step === 2 && (
            ...
            >
              ...
              <div className="mt-6 grid grid-cols-1 gap-2 sm:grid-cols-2">
                {filtered.map((o, i) => {
                  const active = selectedOccupation?.id === o.id;
                  return (
                    <motion.button
                      key={o.id}
                ...
                {search.trim() && filtered.length === 0 && typedOccupation && (
                  <div className="glass rounded-2xl px-5 py-4 text-sm text-muted-foreground sm:col-span-2">
                    We&apos;ll use the closest model match for "{typedOccupation.name}".
                  </div>
                )}
              </div>
            </motion.section>
        ...
        {step < 4 && (
          <div className="mt-12 flex items-center justify-between">
            <span className="text-xs text-muted-foreground">
              {selectedOccupation ? `Selected: ${selectedOccupation.name}` : "Take your time."}
            </span>
            <button
