...
const ForgotPassword = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setSent(true);
      toast.success(response.message || "Reset token generated.");
    } catch (error) {
      const message = error?.response?.data?.detail || "Could not generate reset token.";
      toast.error(message);
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <AuthLayout
      ...
      subtitle="Enter your email and we'll generate a secure reset token for the current backend flow."
      footer={
        <>
          Remembered it?{" "}
          <Link
    ...
    >
      ...
      {sent ? (
        ...
        >
          ...
          <div>
            <div className="font-display text-lg font-medium">Reset token ready</div>
            <p className="mt-1 text-sm text-muted-foreground">
              If an account exists for <span className="font-medium text-foreground">{email}</span>,
              the backend generated a password reset token for this flow.
            </p>
          </div>
          {resetToken ? (
            <div className="w-full rounded-2xl bg-background/60 p-4">
              <div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
                Reset token
              </div>
              <div className="mt-2 break-all font-mono-data text-xs text-foreground">
                {resetToken}
              </div>
            </div>
          ) : null}
